...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	va_temp = va;	
	while (va_temp < va + size) {
		pgtable_entry = boot_pgdir_walk(pgdir, va_temp, 1);
		*pgtable_entry = (pa + va_temp - va) | (perm | PTE_V);
		va_temp += BY2PG;
	}
}
...
page_init(void)
{
	...
	extern char end[];
	for (i = 0; i < npage; i++) {
		if ((u_long)(i*BY2PG) < maxpa) {
			if ((u_long)end + i*BY2PG <freemem) {
				pages[i].pp_ref = 1;
			}
			else {
				pages[i].pp_ref = 0;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    /* Step 1: Get the corresponding page directory entry and page table. */
	pgdir_entryp = pgdir + PDX(va);
	if ((*pgdir_entryp & PTE_V) && (*pgdir_entryp & PTE_R))	{
		pgtable =(Pte *)(KADDR(PTE_ADDR(*pgdir_entryp)));
	}
	else if (create == 1) {
...
page_check(void)
{
    ...
    assert(page_insert(boot_pgdir, pp1, 0x0, 0) == 0);
    // printf("pp0->pp_ref: %d\n", pp0->pp_ref);
	// printf("PTE_ADDR(boot_pgdir[0]): %x, page2pa(pp0):%x\n",PTE_ADDR(boot_pgdir[0]), page2pa(pp0));
