							 ...
							 u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	bcopy((void *)bin, page2kva(p)+offset, BY2PG-offset);
	if ((r = page_insert(env->env_pgdir, p, va, PTE_R|PTE_V))<0) {
	...
	for (i = BY2PG-offset; i < bin_size; i += BY2PG) {
		...
		if ((r = page_insert(env->env_pgdir, p, va+i, PTE_R|PTE_V))<0) {
			panic("page_insert failed!");
			return r;
		}
	}
